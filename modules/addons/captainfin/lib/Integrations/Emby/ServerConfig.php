...
final class ServerConfig
{
    ...
    public static function fromWhmcs(array $params): self
    {
        ...
        if ($hostname === '') {
            throw new \InvalidArgumentException('Emby server hostname is required.');
        }
        if ($apiKey === '') {
            throw new \InvalidArgumentException('Enter the Emby API key in the WHMCS server Password field.');
        }
        if (preg_match('/[\r\n]/', $apiKey)) {
            throw new \InvalidArgumentException('Emby API key contains invalid characters.');
        }
        ...
        if (preg_match('#^https?://#i', $hostname)) {
            ...
            if (str_contains($hostname, '/') || str_contains($hostname, '@')) {
                throw new \InvalidArgumentException('Emby server hostname must be a hostname or a full http/https URL.');
            }
    ...
    public static function normalizeBaseUrl(string $value): string
    {
        ...
        if (!is_array($parts) || !isset($parts['scheme'], $parts['host'])) {
            throw new \InvalidArgumentException('Enter a valid Emby http/https URL.');
        }

        $scheme = strtolower((string) $parts['scheme']);
        if (!in_array($scheme, ['http', 'https'], true)) {
            throw new \InvalidArgumentException('Only http and https Emby URLs are allowed.');
        }
        if (isset($parts['user']) || isset($parts['pass']) || isset($parts['fragment']) || isset($parts['query'])) {
            throw new \InvalidArgumentException('Emby URLs may not contain credentials, query strings or fragments.');
        }
        ...
        if ($host === '') {
            throw new \InvalidArgumentException('Emby URL hostname is required.');
        }
